...
```bash
cp .env.example .env
# Fill in CUR_S3_BUCKET, CUR_REPORT_NAME, AWS credentials, and OTEL_EXPORTER_OTLP_HEADERS
docker compose up
```
...
| `OTEL_EXPORTER_OTLP_HEADERS` | Yes | — | Last9 auth header (e.g. `Authorization=Basic <token>`) |
...
**Memory**: CUR Parquet files are loaded fully into memory before aggregation. Large multi-account orgs can produce CUR files of 5–20 GB per billing period. If the container OOMs, reduce `MONTHS_BACK` to `1` or run on a host with sufficient RAM (recommend ≥8 GB for large orgs). Streaming Parquet reads are not currently supported.
