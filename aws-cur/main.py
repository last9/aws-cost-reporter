...
def _compute_amortized(df: pd.DataFrame) -> pd.Series:
    ...
    for item_type, col in _AMORTIZED_COL_BY_TYPE.items():
        if col in df.columns:
            ...
            result.loc[mask] = pd.to_numeric(df.loc[mask, col], errors="coerce").fillna(
                0.0
            )
    return result
...
def aggregate_costs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["date"] = pd.to_datetime(df["line_item_usage_start_date"]).dt.strftime(
        "%Y-%m-%d"
    )
    df["line_item_unblended_cost"] = pd.to_numeric(
        df["line_item_unblended_cost"], errors="coerce"
    ).fillna(0.0)
    df["line_item_usage_amount"] = pd.to_numeric(
        df["line_item_usage_amount"], errors="coerce"
    ).fillna(0.0)
    ...
    for tag_key in COST_ALLOCATION_TAGS:
        ...
        if cur_col in df.columns:
            ...
            log.warning(
                "Cost allocation tag column not found in CUR: %s (tag: %s)",
                cur_col,
                tag_key,
            )
...
def send_otlp_metrics(agg: pd.DataFrame) -> None:
    ...
    for _, row in agg.iterrows():
        ...
        if unblended != 0.0:
            cost_dps.append(
                {"attributes": attrs, "timeUnixNano": time_ns, "asDouble": unblended}
            )
        if amortized != 0.0:
            amortized_dps.append(
                {"attributes": attrs, "timeUnixNano": time_ns, "asDouble": amortized}
            )
        if qty != 0.0:
            usage_dps.append(
                {"attributes": attrs, "timeUnixNano": time_ns, "asDouble": qty}
            )
    ...
    log.info(
        "Exported %d unblended + %d amortized + %d usage data points",
        len(cost_dps),
        len(amortized_dps),
        len(usage_dps),
    )
...
# Scheduling handled by the poll loop (POLL_INTERVAL_SECONDS). No Lambda mode for CUR.
...
def poll(s3: Any) -> None:
    ...
    if not periods:
        log.warning(
            "No billing period folders found at s3://%s/%s",
            CUR_S3_BUCKET,
            _report_prefix(),
        )
        return
    ...
    for period_prefix in periods[:MONTHS_BACK]:
        try:
            ...
            billing_start = manifest.get("billingPeriod", {}).get(
                "start", period_prefix
            )
            log.info(
                "Billing period: %s (%d file(s))",
                billing_start,
                len(manifest.get("reportKeys", [])),
            )
            ...
            log.warning(
                "Manifest not found for %s — CUR may still be generating", period_prefix
            )
